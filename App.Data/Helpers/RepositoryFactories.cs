    ...
    public class RepositoryFactories
    {
        ...
        private IDictionary<Type, Func<DbContext, object>> GetAppFactories()
        {
            return new Dictionary<Type, Func<DbContext, object>>
                {
                   {typeof(IActivityLogRepository), DbContext => new ActivityLogRepository(DbContext)},
                   {typeof(IFollowRepository), DbContext => new FollowRepository(DbContext)},
                   {typeof(IGameRepository), DbContext => new GameRepository(DbContext)},
                   {typeof(IGettinListRepository), DbContext => new GettinListRepository(DbContext)},
                   {typeof(ISocialLinkRepository), DbContext => new SocialLinkRepository(DbContext)},
                   {typeof(IUserRepository), DbContext => new UserRepository(DbContext)},
                   {typeof(IVendorRepository), DbContext => new VendorRepository(DbContext)}
                };
        }
